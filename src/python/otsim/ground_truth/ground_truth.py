...
class GroundTruth:
  def __init__(self: GroundTruth, pub: str, el: ET.Element):
    ...
    # Print for debugging
    print(f"Connecting to ElasticSearch at {self.elastic}")
  ...
  def handle_msgbus_status(self: GroundTruth, env: Envelope):
    ...
    if status:
      ...
      for point in status.get('measurements', []):
        ...
        # Print to see what document is being sent
        print(f"Sending document to index {index}: {json.dumps(doc,indent=2)}")
        ...
        if not resp.ok:
          ...
          print(f"Successfully sent document to ElasticSearch with status {resp.status_code}")
  ...
  def __ensure_index_template(self: GroundTruth):
    ...
    # Add print to log the index template creation
    print(f"Creating index template at {self.elastic}/_index_template/{self.index_base}")
    print(f"Index template payload: {json.dumps(template, indent=2)}")
    # Send the template
    ...
    if not resp.ok:
      ...
      print(f"Successfully created index template with status {resp.status_code}")
